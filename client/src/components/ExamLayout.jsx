...
import React, { useCallback, useContext, useEffect, useState, Suspense, useMemo } from "react"
...
import VioletMsg from "./modals/VioleteMsg"
...
function ExamLayout() {
    ...
    useEffect(() => {
        ...
        async function getExamPaper() {
            ...
            if (flag) {
                ...
                response.json().then(data => setPaper(data)).catch(err => console.error(err))
            }
        }
    ...
    const questionNo = useCallback(Array.from({ length: paper?.totalQuestions }, (v, i) => i), [paper])
    ...
    const displayNumberPattern = useMemo(() => questionNo?.map((e, k) => {
    ...
    return (
            ...
            <QuestionContext.Provider value={{currentQuestion, setCurrentQuestion, paper, setPaper}}>
                <div className="w-full pt-20  flex select-none" >
                    ...
                    <div className="w-[60%] pt-12 relative">
                        {/* Actual question */}
                        <Suspense fallback={<h3 className="text-xl text-center fixe left-[50%] top-[50%] translate-x-[-50%] translate-y-[50%]">Loading questions...</h3>}>
                            {paper && <Question />}
                        </Suspense>
                    ...
                    <div className="w-[20%] bg-slate-100 pt-12">
                        ...
                        <div className="flex justify-center mt-36">
                            <button className="px-4 py-1 rounded-md bg-green-600 text-white font-semibold active:bg-green-700" onClick={() => setSubmit(true)}>Submit</button>
                        </div>
                    </div>
                </div>
